...
start(uint32_t *modulep, void *physbase, void *physfree)
{
    ...
    init_idt();
    checkAllBuses();
    hba_port_t port = ioports[1];
    intitialise(&port);
    uint16_t* rd_buf = (uint16_t*)0x3affffff;
    uint16_t* wt_buf = (uint16_t*)0x3bffffff;
    for(int write_block=0;write_block<100;write_block++)
    {
        for(int i=0;i<4096;i++)
            wt_buf[i]=(char)write_block;
        read_write( &port, write_block*8,0, 8, wt_buf, 0);
        read_write( &port, write_block*8, 0,8, rd_buf, 1);
        //str_cmp
       for(int i=0;i<4096;i++)
        {
